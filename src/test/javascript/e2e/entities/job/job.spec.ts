...
describe('Job e2e test', () => {
  ...
  it('should load Jobs', async () => {
    ...
    expect(await jobComponentsPage.getTitle()).to.eq('studelyApp.job.home.title');
  });

  it('should load create Job page', async () => {
    await jobComponentsPage.clickOnCreateButton();
    jobUpdatePage = new JobUpdatePage();
    expect(await jobUpdatePage.getPageTitle()).to.eq('studelyApp.job.home.createOrEditLabel');
  ...
  it('should delete last Job', async () => {
    ...
    expect(await jobDeleteDialog.getDialogTitle()).to.eq('studelyApp.job.delete.question');
